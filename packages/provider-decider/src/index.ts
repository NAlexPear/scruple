...
export interface DeciderProviderOptions {
  apiKey?: string;
  baseURL?: string;
  model?: string;
  concurrency?: number;
...
export const deciderProvider = (options: DeciderProviderOptions = {}): DecisionProvider => {
  if (options.apiKey !== undefined && options.apiKey.trim().length === 0) {
    throw new TypeError("Decider apiKey must not be empty");
  }
  const model = options.model ?? "decider-4b-v2.1";
