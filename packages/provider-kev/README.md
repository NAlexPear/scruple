...
Kev serves the System One API from open weights. The provider refuses a non-loopback `baseURL` unless
`allowRemote` is `true`, so code stays on your machine by default. It queues requests beyond
`concurrency`, which defaults to `1` because Kev answers one request at a time.
